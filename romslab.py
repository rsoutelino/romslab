        ...
        if self._RUN_AVERAGED or self._TIME_AVERAGED:
            for termx, termy in zip(self.keys_xi, self.keys_eta):
                ...
                if print_terms:
                    print("\n")
                    print(
                        "%s (min,mean,max)   %.1e  %.1e  %.1e"
                        % (
                            termx,
                            np.nanmin(np.abs(Termx)),
                            np.nanmean(np.abs(Termx)),
                            np.nanmax(np.abs(Termx)),
                        )
                    )
                    print(
                        "%s (min,mean,max)   %.1e  %.1e  %.1e"
                        % (
                            termy,
                            np.nanmin(np.abs(Termy)),
                            np.nanmean(np.abs(Termy)),
                            np.nanmax(np.abs(Termy)),
                        )
                    )
                    print("\n")
            ...
            print("\n")
            print(
                "XI  (min,mean,max)   %.1e  %.1e  %.1e"
                % (
                    np.nanmin(np.abs(residuex)),
                    np.nanmean(np.abs(residuex)),
                    np.nanmax(np.abs(residuex)),
                )
            )
            print(
                "ETA (min,mean,max)   %.1e  %.1e  %.1e"
                % (
                    np.nanmin(np.abs(residuey)),
                    np.nanmean(np.abs(residuey)),
                    np.nanmax(np.abs(residuey)),
                )
            )
            print("++++++++++++++++++++++++++++++++++++")
        else:
            for n in xrange(self.nt):
                ...
                print("================================================")
                print(
                    "INSTANTANEOUS M2 balance residues (record %d/%d)"
                    % (n + 1, self.nt)
                )
                print("================================================")
                print("\n")
    ...
    def __init__(self, outname):
        ...
        for name, var in zip(namelist, varlist):
            try:
                ...
                print(
                    "WARNING: ROMS output NetCDF file must contain the \
...
not work properly.\n"
    ...
    def hslice(
        ...
        if velkey:
            ...
            u = 0 * self.lonu
            v = 0 * self.lonv  # initializing arrays
            if nk == -1:
            ...
            u = griddata(
                self.lonu.ravel(), self.latu.ravel(), u.ravel(), self.lonr, self.latr
            )
            v = griddata(
                self.lonv.ravel(), self.latv.ravel(), v.ravel(), self.lonr, self.latr
            )
        ...
        if trkey:
            titlestr += "%s " % (tr)
            self.pcolor = m.pcolormesh(
                self.mlon, self.mlat, tracer, vmin=trmin, vmax=trmax
            )
        ...
        if velkey:
            ...
            self.vAtZ = v
            titlestr += "vel "
            self.quiver = m.quiver(
                self.mlon[::vskip, ::vskip],
                self.mlat[::vskip, ::vskip],
                u[::vskip, ::vskip],
                v[::vskip, ::vskip],
                scale=vsc,
            )
            vmax = np.sqrt(u ** 2 + v ** 2).max()
            self.qkey = plt.quiverkey(
                self.quiver,
                0.6,
                -0.05,
                vmax,
                r"$ %.0f cm s^{-1}$" % (vmax * 100),
                labelpos="W",
        ...
        np.sqrt(
            ...
            + (
                (
                    ...
                    - (np.sin(lat1) * np.cos(lat2) * np.cos(l))
                )
                ** 2
            )
        )
...
def get_angle(latu, lonu, argu1="wgs84"):
    ...
    TPSI2 = (1.0 - E * E) * np.tan(PHI2) + E * E * xnu1 * np.sin(PHI1) / (
        xnu2 * np.cos(PHI2)
    )
...
def add_topo(lon, lat, pm, pn, toponame):
    ...
    x, y, z = subset(
        x, y, z, lon.min() - 1, lon.max() + 1, lat.min() - 1, lat.max() + 1
    )
...
def rotfilter(h, maskr, hmax_coast, rmax):
    ...
    while r > rmax:
        ...
        h[1:Mm, 1:Lm] = h[1:Mm, 1:Lm] + nu * (
            (fx[1:Mm, 1:Lm] - fx[1:Mm, 0:Lmm]) + (fy[1:Mm, 1:Lm] - fy[0:Mmm, 1:Lm])
        )
        h[0, :] = h[1, :]
        h[M - 1, :] = h[Mm - 1, :]
        h[:, 0] = h[:, 1]
...
def hanning_smoother_coef2d(h, coef):
    ...
    h[1:Mm, 1:Lm] = (
        coef[1:Mm, 1:Lm]
        * (h[0:Mmm, 1:Lm] + h[2:M, 1:Lm] + h[1:Mm, 0:Lmm] + h[1:Mm, 2:L])
...
def FX(h):
    ...
    fx[1:Mm, :] = (h[1:Mm, 1:L] - h[1:Mm, 0:Lm]) * 5 / 6 + (
        h[0:Mmm, 1:L] - h[0:Mmm, 0:Lm] + h[2:M, 1:L] - h[2:M, 0:Lm]
    ) / 12
...
def FY(h):
    ...
    fy[:, 1:Lm] = (h[1:M, 1:Lm] - h[0:Mm, 1:Lm]) * 5 / 6 + (
        h[1:M, 0:Lmm] - h[0:Mm, 0:Lmm] + h[1:M, 2:L] - h[0:Mm, 2:L]
    ) / 12
...
def rx1(z_w, rmask):
    ...
    r = np.maximum(
        np.maximum(zx[:, :, :-1], zx[:, :, 1:]), np.maximum(zy[:, :-1, :], zy[:, 1:, :])
    )
...
def rx0(h, rmask):
    ...
    rx0 = np.maximum(
        np.maximum(hx[:-1, :], hx[1:, :]), np.maximum(hy[:, :-1], hy[:, 1:])
    )
...
def stretching(sc, Vstretching, theta_s, theta_b):
    ...
    if Vstretching == 1:
        ...
        cff1 = 1.0 / np.sinh(theta_s)
        cff2 = 0.5 / np.tanh(0.5 * theta_s)
        C = (1.0 - theta_b) * cff1 * np.sinh(theta_s * sc) + theta_b * (
            cff2 * np.tanh(theta_s * (sc + 0.5)) - 0.5
        )
        return C
...
def get_zlev(h, sigma, hc, sc, ssh=0.0, Vtransform=2):
    if Vtransform == 1:  # ROMS 1999
        ...
        if len(h.shape) > 1:
            z0 = (hc * sc[:, None, None] + sigma[:, None, None] * h[None, :, :]) / (
                h[None, :, :] + hc
            )
        else:
